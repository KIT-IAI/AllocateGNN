"""
WorldCoverFetcher - ESA WorldCover v200 GeoTIFF download

Uses GEE to download a single image (not an ImageCollection), with no
time/cloud filtering/compositing.
Tiled download infrastructure adapted from ImageFetcher.py.
"""
import hashlib
import logging
import math
...
def _estimate_region_size(
    ...
    # WorldCover is uint8, 1 byte/pixel
    estimated_bytes = width_px * height_px * n_bands * 1
...
def _compute_tile_grid(
    ...
    n_bands: int,
    scale_m: int,
    target_bytes: int = 9_600_000,
) -> List[Tuple[float, float, float, float]]:
    """Split the projected region into a tile grid (copied from ImageFetcher._compute_tile_grid)"""
    # uint8: 1 byte/pixel/band; overestimating is safer than underestimating,
    # to ensure tiles don't exceed the GEE limit
    tile_px = int(math.floor(math.sqrt(target_bytes / (n_bands * 1))))
...
def _download_and_merge_tiles(
    image, tile_bboxes, bands, scale_m, crs, output_path
) -> str:
    """Download the GEE image tile by tile and merge into a single GeoTIFF (copied from ImageFetcher._download_and_merge_tiles)"""
    import requests
    import rasterio
...
@fetcher_registry.register("worldcover", description="ESA WorldCover 10m download")
class WorldCoverFetcher(BaseFetcher):
    """
    Downloads an ESA WorldCover v200 GeoTIFF from GEE.

    Config fields:
        gee_image: GEE Image ID (default "ESA/WorldCover/v200")
        band: band name (default "Map")
        scale_m: output resolution (default 10)
        output_crs: output CRS (default "EPSG:27700")
    """

    def __init__(self, config: dict):
        super().__init__(config)
        self.gee_image = config.get("gee_image", "ESA/WorldCover/v200/2021")
        self.band = config.get("band", "Map")
    ...
    def fetch(self, region_bounds: Any, cache_dir: str) -> str:
        """
        Download the WorldCover GeoTIFF and return the file path.
        """
        cache_key = self.get_cache_key(region_bounds)
        wc_cache_dir = os.path.join(cache_dir, "worldcover")
        os.makedirs(wc_cache_dir, exist_ok=True)
        cache_path = os.path.join(wc_cache_dir, f"{cache_key}_worldcover.tif")

        if os.path.exists(cache_path) and os.path.getsize(cache_path) > 1024:
            logger.info(f"WorldCover cache hit: {cache_path}")
            return cache_path

        # Initialize GEE
        _initialize_gee()

        # Build the GEE Image (a single image, not an ImageCollection)
        ...
        image = ee.Image(self.gee_image).select(self.band)
        ...
        estimated_bytes, proj_bbox = _estimate_region_size(
            region_bounds, n_bands, self.scale_m
        )

        # The GEE hard limit is roughly 48 MB, but the estimate is about 50% of the
        # actual value (BNG projection bbox underestimation), so the threshold is
        # set to 20 MB (corresponding to ~40 MB actual, leaving a safety margin)
        single_threshold = 8_000_000

        if estimated_bytes <= single_threshold:
            logger.info(f"WorldCover region is small (~{estimated_bytes / 1e6:.0f} MB), downloading directly...")
            import requests
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        wait_sec = 5 * (3 ** attempt)
                        logger.warning(f"Download failed, retrying {attempt+1}/{max_retries} (waiting {wait_sec}s)... {e}")
                        time.sleep(wait_sec)
                    else:
                        raise RuntimeError(f"WorldCover download failed (after {max_retries} retries): {e}") from e
        else:
            tile_bboxes = _compute_tile_grid(proj_bbox, n_bands, self.scale_m)
            logger.info(f"WorldCover region is large (~{estimated_bytes / 1e6:.0f} MB), downloading in {len(tile_bboxes)} tiles...")
            _download_and_merge_tiles(
                image, tile_bboxes, bands, self.scale_m, self.output_crs, cache_path
            )

        size_mb = os.path.getsize(cache_path) / 1024 / 1024
        logger.info(f"WorldCover download complete: {cache_path} ({size_mb:.1f} MB)")
        return cache_path
    ...
    def get_cache_key(self, region_bounds: Any) -> str:
        ...
        key_str = f"{bounds_tuple}_{self.gee_image}_{self.band}_{self.scale_m}"
