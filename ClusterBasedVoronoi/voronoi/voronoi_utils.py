...
__all__ = ['calcu_overlap_dict', 'assign_points_to_polygons']
...
def assign_points_to_polygons(polygons_gdf, points):
    # Initialize the 'points' column in polygons_gdf to store lists of point indices
    polygons_gdf['points'] = [[] for _ in range(len(polygons_gdf))]

    # Iterate over each point and check which polygon it is within
    for point_idx, point in points.iterrows():
        for poly_idx, polygon in polygons_gdf.iterrows():
            if polygon['geometry'].contains(point['geometry']):
                polygons_gdf.at[poly_idx, 'points'].append(point_idx)
